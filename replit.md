...
- **October 07, 2025**: Major improvements and fixes:
  ...
  - **Channel smoothness improvements**: Implemented shared bandwidth meter with 2.8 Mbps initial estimate to maintain bandwidth learning across channel switches. Added setPrioritizeTimeOverSizeThresholds for time-based buffer optimization. Added adaptive bitrate hysteresis (10s delay for quality increase, 25s for decrease) to prevent rapid quality switching that causes stuttering on multi-bitrate streams. Fixed memory leak by using applicationContext.
