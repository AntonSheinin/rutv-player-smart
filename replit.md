...
- **October 08, 2025**: NextLib FFmpeg integration for audio and video decoding:
  - **Replaced Jellyfin FFmpeg**: Migrated from `org.jellyfin.media3:media3-ffmpeg-decoder` to NextLib's `io.github.anilbeesetti:nextlib-media3ext` for full audio and video FFmpeg support
  - **Separate FFmpeg controls**: Added independent toggles for "Use FFmpeg Audio Decoder" and "Use FFmpeg Video Decoder" in settings
  - **No decoder fallback**: Removed fallback to hardware decoders when FFmpeg is selected (EXTENSION_RENDERER_MODE_PREFER vs OFF only)
  - **Playlist indicators**: Added filename/URL display next to "Load Playlist" and "Load from URL" buttons showing current source
  - **Extended NextRenderersFactory**: Custom factory extends NextLib's NextRenderersFactory to control audio/video FFmpeg usage independently
  - **Kotlin 2.1.21 upgrade**: Updated from Kotlin 1.9.25 to 2.1.21 to match NextLib's exact Kotlin version
...
- **Codec Support**: Utilizes NextLib's `nextlib-media3ext` for comprehensive audio and video FFmpeg decoding support (MP2, AAC, H.264, HEVC, VP8, VP9, etc.). Hardware MediaCodec decoders available as alternative.
...
- **FFmpeg Audio/Video Toggles**: Separate user-configurable options to enable/disable FFmpeg software decoding for audio and video independently. Uses EXTENSION_RENDERER_MODE_PREFER when enabled, OFF when disabled. No fallback to hardware decoders when FFmpeg is selected (as requested).
...
- **NextLib FFmpeg Decoder**: `io.github.anilbeesetti:nextlib-media3ext` (provides both audio and video FFmpeg decoding support for formats like MP2, AAC, H.264, HEVC, VP8, VP9, etc.).
